...
<template>
    <PrimeCard unstyled
        ...
        :class="{
            ...
            'opacity-30 bg-emerald-500 text-white pointer-events-none': props.term.state === AnswerState.Correct,
        }"  
        :disabled='(props.term.state === AnswerState.Correct) || (props.term.state === AnswerState.Incorrect && props.term.isSelected)'
        @click='handleClick(props.term, lang)'>
        <template #title>
            <h1 class='text-xl font-bold text-center lg:text-3xl lg:p-4'>{{ props.term.phrase }}</h1>
        </template>
    </PrimeCard>
</template>